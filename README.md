...
**Lato dipendente** (`/`): sceglie il proprio nome dalla griglia, inserisce il PIN a 4 cifre. In cima vede subito un riquadro con le proprie **ore da conteggiare** (dall'ultimo pagamento registrato ad oggi, o "da sempre" se non è mai stato pagato) — solo le ore, mai il costo orario, che resta visibile solo al Titolare — e sotto i turni di oggi. Può registrare un nuovo turno (bloccato non appena salvato: non è più modificabile né cancellabile da lì), o toccare "Turno di un giorno passato": sceglie una data da un calendario a comparsa (mesi in italiano, giorni futuri disabilitati), e se quel giorno ha già un turno lo seleziona per correggerlo (motivo obbligatorio); se invece è vuoto (tipicamente tutti i giorni prima di iniziare a usare l'app) propone gli orari lavorati (nota facoltativa) — utile per compilare rapidamente lo storico arretrato. Entrambe restano "in attesa" finché il Titolare non decide; se accetta una proposta di giorno vuoto, il turno viene creato automaticamente. Può anche richiedere un'assenza: sceglie il periodo toccando il primo e l'ultimo giorno sullo stesso calendario (o lo stesso giorno due volte per un'assenza di un giorno solo), poi indica se è giornata intera o solo una fascia oraria di quel giorno (motivo facoltativo). Quando il Titolare accetta o rifiuta una richiesta, al successivo accesso col PIN il dipendente vede un avviso con l'esito e l'eventuale nota lasciata dal Titolare; una volta letto (pulsante "Ho capito") non ricompare più. Se invece il Titolare gli propone un turno (es. una sostituzione), al successivo accesso col PIN il dipendente vede subito la proposta (data, orario, nota) e deve accettarla o rifiutarla (motivo obbligatorio in caso di rifiuto) prima di continuare.

**Lato Titolare** (`/titolare`, codice a 6 cifre): tab **Richieste** (modifiche turno + assenze, con contatore, nota facoltativa e pulsanti accetta/rifiuta — la nota, se scritta, arriva al dipendente insieme all'esito; in cima, il pulsante **Proponi turno** apre invece il flusso opposto: il Titolare sceglie dipendente, data, orario e una nota facoltativa — es. il motivo di una sostituzione — e il turno proposto appare tra "Turni proposti da te" in attesa che il dipendente lo accetti o lo rifiuti; se accetta, il turno viene creato automaticamente; in entrambi i casi il Titolare riceve un'email con l'esito, se le notifiche sono configurate), **Da pagare** (ore non ancora pagate per dipendente, calcolate dall'ultimo pagamento registrato in poi — se il dipendente ha un giorno di paga impostato, mostra anche la prossima data prevista; cliccando su un dipendente (anche a 0h) si apre l'elenco dei turni che compongono il totale, modificabili o eliminabili sul posto, e l'elenco dei pagamenti registrati per lui, ciascuno modificabile (cambia la data "pagato fino al") o annullabile del tutto — utile se un pagamento è stato segnato per errore o su un periodo sbagliato: correggendolo o annullandolo, le ore coinvolte tornano subito visibili come da pagare; "Segna come pagato" salva solo la data fino a cui si è pagato, non un elenco dettagliato), **Dipendenti** (nome, PIN, costo orario — mai visibile al dipendente — giorno di paga previsto, email facoltativa per le notifiche, attivo/disattivato), **Turni** (apre, modifica o elimina qualsiasi turno di qualsiasi dipendente, anche passato, o ne aggiunge uno dimenticato), **Riepilogo** (vista mensile navigabile avanti/indietro, ore e costo per dipendente — cliccando su un dipendente si apre e modifica l'elenco dei turni del mese — resta consultabile anche dopo il pagamento, esportabile in PDF/Excel per singolo dipendente o aggregato).
...
## 5. Notifiche email (facoltativo)

**Al Titolare**, ogni volta che un dipendente invia una richiesta (modifica turno, nuovo turno passato, assenza):
...
**Al dipendente**, quando il Titolare accetta o rifiuta una sua richiesta: basta impostargli un'email nel modulo **Dipendenti** (campo facoltativo). Chi non ha un'email impostata continua a vedere l'esito solo in-app al prossimo accesso col PIN, come sempre.

Il mittente delle email è impostato nel file `schema.sql` (funzione `send_email`, riga con `'ORE <notifiche@fitpointactive.com>'`): con il mittente di test di Resend (`onboarding@resend.dev`) si può mandare solo alla propria email dell'account Resend, non a indirizzi esterni come quelli dei dipendenti — per quello serve verificare un dominio proprio su Resend (**Domains** → **Add Domain**, poi aggiungere i record DNS indicati) e aggiornare quella riga con l'indirizzo scelto sul proprio dominio. Se non configuri nulla, l'app funziona esattamente come prima: è tutto opzionale e non blocca mai l'invio/risoluzione di una richiesta anche se l'email non parte.
