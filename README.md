...
2. Nel progetto, apri **SQL Editor** → **New query**, incolla tutto il contenuto di [`supabase/schema.sql`](./supabase/schema.sql) e premi **Run**. Questo crea le tabelle (`employees`, `shifts`, `payments`, `edit_requests`, `absence_requests`, `settings`), le policy di sicurezza, le funzioni RPC e il codice Titolare iniziale (`123456`).
